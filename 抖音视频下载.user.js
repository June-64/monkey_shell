...
(function () {
    ...
    function insertControlPanel() {
      ...
      downloadAllButton.style.fontSize='10px'
      downloadAllButton.addEventListener("click", downloadAll);
  
      const downloadAllCoverButton = document.createElement("button");
      downloadAllCoverButton.innerHTML = "下载全部(封面)";
      downloadAllCoverButton.style.flex = "1";
      downloadAllCoverButton.style.marginRight = "5px";
      downloadAllCoverButton.style.backgroundColor = "#28a745";
      downloadAllCoverButton.style.color = "white";
      downloadAllCoverButton.style.border = "none";
      downloadAllCoverButton.style.borderRadius = "5px";
      downloadAllCoverButton.style.cursor = "pointer";
      downloadAllCoverButton.style.fontSize='10px';
      downloadAllCoverButton.addEventListener("click", downloadAllCover);
      ...
      downloadSelectedButton.style.fontSize='10px';
      downloadSelectedButton.addEventListener("click", downloadSelected);
  
      const downloadSelectedCoverButton = document.createElement("button");
      downloadSelectedCoverButton.innerHTML = "下载选中(封面)";
      downloadSelectedCoverButton.style.flex = "1";
      downloadSelectedCoverButton.style.marginLeft = "5px";
      downloadSelectedCoverButton.style.backgroundColor = "#ffc107";
      downloadSelectedCoverButton.style.color = "white";
      downloadSelectedCoverButton.style.border = "none";
      downloadSelectedCoverButton.style.borderRadius = "5px";
      downloadSelectedCoverButton.style.cursor = "pointer";
      downloadSelectedCoverButton.style.fontSize='10px'
      downloadSelectedCoverButton.addEventListener("click", downloadSelectedCover);
      ...
      clearAllButton.style.fontSize='10px'
      ...
      clearSelectedButton.style.fontSize='10px'
      clearSelectedButton.addEventListener("click", () => {
        videoList = videoList.filter((video) => !video.checkbox.checked);
        panel.innerHTML = "";
        videoList.forEach((video) =>
          panel.appendChild(video.checkbox.parentNode)
        );
        // 对序号重新排序
        count = 0;
        videoList.forEach((video) => {
      ...
      buttonContainer.appendChild(downloadAllCoverButton);
      buttonContainer.appendChild(downloadAllButton);
      buttonContainer.appendChild(downloadSelectedCoverButton);
    ...
    // 插入视频信息
    function insertVideoInfo(title, url, cover) {
      // 如果视频已经存在，则不再插入
      if (
        videoList.some(
          (video) => video.url === url || (title && video.title === title)
        )
      ) {
        return;
      }
    ...
    // 下载封面图片并保存到本地
    function downloadCover(title, url) {
      // 确保 URL 使用 HTTPS
      ...
      xhr.responseType = "blob"; // 设置响应类型为 blob，以便处理图片文件
  
      xhr.onload = function () {
        if (xhr.status === 200) {
          // 请求成功，创建图片文件的 Blob URL
            ...
            if (
              (this.readyState === 4 &&
                ...
                (this.responseURL.includes("/aweme/v1/web/aweme/post/") ||
                  this.responseURL.includes("/aweme/v2/web/feed/"))) ||
              this.responseURL.includes("/v1/web/aweme/favorite/")
            ) {
        ...
        response.aweme_list.forEach((aweme) => {
          try {
            if (aweme.aweme_type !== 101) {
              ...
              const prefix = aweme.mix_info?.statis?.current?.episode
                ? `第${aweme.mix_info?.statis?.current?.episode}集：`
                : "";
              // 加上前缀，去掉所有的空白字符
              title = (prefix + title).replace(/\s/g, "");
              const cover = aweme.video.origin_cover.url_list[0];
              if (videoUrl) {
                insertVideoInfo(title, videoUrl, cover);
              }
            }
          } catch (e) {
    ...
    function addDownloadButton() {
      ...
      // 如果有下载按钮则不再添加
      if (
        rightGrid.querySelector(".download-button") ||
        rightGrid.querySelector(".xgplayer-playback-setting") === null
      ) {
        return;
      }
      if (rightGrid) {
        ...
        downloadButton.addEventListener("click", () => {
          const videoContainer = targetElement.querySelector(
            ".xg-video-container"
          );
          if (videoContainer) {
            const videoSource = videoContainer.querySelector("video source");
            const title = targetElement
              .querySelector('[data-e2e="video-desc"] span')
            ...
            if (videoSource && videoSource.src) {
              ...
              if (!isDownload) showToast("未找到视频链接");
            }
          } else {
